#!/bin/bash
source ~/bin/display_vars
xrandr --output $DESK_DISPLAY_PRI --mode $PRI_RES --rate $PRI_RATE --scale-from $OSRS_RES --primary --pos 0x180 --output $DESK_DISPLAY_SEC --mode $SEC_RES --rate $SEC_RATE --scale-from $SEC_RES --pos 1280x0
xrandr --output $TV_DISPLAY --off
xfconf-query -c xfce4-panel -p /panels/panel-1/output-name -s monitor-1
#killall xfce4-panel
#xfce4-panel &
#kilall compton
#compton --backend glx --vsync opengl-swc &
pacmd set-default-sink "$DESK_AUDIO"
/home/austin/bin/pulseaudioswitch.sh "$DESK_AUDIO"
